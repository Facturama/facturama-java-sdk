...
public class SampleApiMultiemisor {
    ...
    public static void principal() {
        ...
        try {
            ...
            // Ejemplo de administración de CSDs (descomenta la linea para incluirlo en la
            // ejecución)
            // sampleCsd(facturama);
    ...
    private static void sampleCsd(FacturamaApiMultiemisor facturama) throws IOException, FacturamaException, Exception {
        ...
        // System.out.println( "Listado de todos los Certificados que tiene cargados el
        // usuario" );
        ...
        // System.out.println( "Mostrado del certificado especìfico para el RFC
        // EKU9003173C9" );
        // System.out.println();
    ...
    private static Csd sampleCsdCreate(FacturamaApiMultiemisor facturama)
            throws IOException, FacturamaException, Exception {
        Csd newCsd = new Csd();
        newCsd.setCertificate(
    ...
    // Test CFDI 3.3
    private static void sampleCfdi(FacturamaApiMultiemisor facturama)
            throws IOException, FacturamaException, Exception {
        ...
        // com.Facturama.sdk_java.Models.Response.Cfdi cfdiCreated =
        // facturama.Cfdis().Create3(cfdi); // CFDI 4.0 Disponible hasta el 30 de Junio
        // 2022

        System.out.println("Se creó exitosamente el cfdi con el folio fiscal: "
                + cfdiCreated.getComplement().getTaxStamp().getUuid()); // CFDI 3.3
        // System.out.println( "Se creó exitosamente el cfdi con el folio fiscal: " +
        // cfdiCreated.getComplement().getTaxStamp().getUuid() ); // cfdi 4.0 test
        ...
        CancelationStatus response = facturama.Cfdis().Remove(cfdiCreated.getId(), "02",
                "d8e34bab-5bd4-4788-bde2-1428dc469e10");
        ...
        if (strCanceled.equals(response.getStatus())) {
            System.out.println("Se ha cancelado exitosamente el cfdi con el folio fiscal: "
                    + cfdiCreated.getComplement().getTaxStamp().getUuid());
        } else if (strPending.equals(response.getStatus())) {
            System.out.println(
                    "La factura está en proceso de cancelación, pueden pasar hasta 72 horas para que se considere cancelada.");
        } else {
            System.out.println("Algo ha pasado, que el CFDI no se ha podido cancelar. Revisa el mensaje: "
                    + response.getMessage());
        }
    ...
    private static void sampleCfdi40(FacturamaApiMultiemisor facturama)
            throws IOException, FacturamaException, Exception {
    ...
    private static com.Facturama.sdk_java.Models.Request.CfdiLite createCfdi(FacturamaApiMultiemisor facturama,
            Currency currency)
            throws IOException, FacturamaException, Exception {
    ...
    private static com.Facturama.sdk_java.Models.Request.CfdiLite createCfdi40(FacturamaApiMultiemisor facturama,
            Currency currency)
            throws IOException, FacturamaException, Exception {
        com.Facturama.sdk_java.Models.Request.CfdiLite cfdi = new com.Facturama.sdk_java.Models.Request.CfdiLite();
    ...
    private static Item addTaxesToItem(Item item, Double numberOfDecimals) {
        ...
        tax.setTotal(Math.round((/* cambie el baseAmount */tax.getBase() * tax.getRate()) * numberOfDecimals)
                / numberOfDecimals);
    ...
    private static void samplePaymentComplement(FacturamaApiMultiemisor facturama)
        ...
        com.Facturama.sdk_java.Models.Request.CfdiLite paymentComplementModel = createModelCfdiPaymentComplement(
                facturama, cfdiInicial);
        ...
        // com.Facturama.sdk_java.Services.Multiemisor.CfdiService.InvoiceType.IssuedLite,
        // cfdiInicial.getId()));
        // System.out.println(facturama.Cfdis().SendEmail("ejemplo@ejemplo.mx",
        // com.Facturama.sdk_java.Services.Multiemisor.CfdiService.InvoiceType.IssuedLite,
        // paymentComplement.getId()));
     ...
     * de expedición
     */
    private static com.Facturama.sdk_java.Models.Request.CfdiLite createModelCfdiGeneral(
     ...
     * de expedición
     */
    private static com.Facturama.sdk_java.Models.Request.CfdiLite createModelCfdiPaymentComplement(
            FacturamaApiMultiemisor facturama, com.Facturama.sdk_java.Models.Response.Cfdi cfdiInicial)
            throws IOException, FacturamaException, Exception {
    ...
    private static void TestCancel(FacturamaApiMultiemisor facturama)
            throws IOException, FacturamaException, Exception {
    ...
    private static com.Facturama.sdk_java.Models.Request.CfdiLite addStaticItemsToCfdi(
            FacturamaApiMultiemisor facturama, com.Facturama.sdk_java.Models.Request.CfdiLite cfdi)
            throws IOException, FacturamaException, Exception {
    ...
    private static void sampleCustomersValidate(FacturamaApiMultiemisor facturama)
            throws IOException, FacturamaException, Exception {
        System.out.println("----- Ejemplo de validación de datos -----");
